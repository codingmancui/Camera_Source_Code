...
public class SettingGenerator {
    ...
    //wangleiquan for Video quality(start)
    private void restoreSetting(int row, String value, int cameraId) {
        ListPreference pref = getListPreference(row, cameraId);
        if (pref != null && pref.findIndexOfValue(value) == NOT_FOUND) {
            if (pref.getEntryValues() != null && pref.getEntryValues().length > 0) {
                pref.setValueIndex(pref.getEntryValues().length - 1);
            }
        }
    }

    private ListPreference getListPreference(int row, int cameraId) {
    ...
    //wangleiquan for Video quality(end)
    ...
    public void restoreSetting(int cameraId) {
        ...
        if (settingItems != null) {
            for (int i = 0; i < settingItems.size(); i++) {
                SettingItem settingItem = settingItems.get(i);
                String value = settingItem.getDefaultValue();
                //wangleiquan for Video quality(start)
                if (ProjectConfig.FEATURE_VIDEO_QUALITY_DEFAULT_HIGHTEST
                    && settingItem.getSettingId() == SettingConstants.ROW_SETTING_VIDEO_QUALITY) {
                    restoreSetting(settingItem.getSettingId(), value, cameraId);
                    break;
                }
                //wangleiquan for Video quality(end)
    ...
    private void updateSettingItem(SettingItem settingItem, ListPreference preference) {
        ...
        case SettingConstants.ONLY_IN_PEFERENCE:
            ...
            //add by zhangfan for SWYQ-596 start
            if (mCameraId == 1 &&
                    settingItem.getKey().equals(SettingConstants.KEY_CONTINUOUS_NUMBER)) {
                settingItem.setDefaultValue("20");
                settingItem.setValue("20");
            }
            //add by zhangfan for SWYQ-596 end
            break;
        default:
            break;
        }
    ...
    private boolean filterPreference(final ListPreference preference) {
        ...
        case SettingConstants.ROW_SETTING_3DNR:
            if (ProjectConfig.XC_FLY_CAMERA_VIDEO_SUPPORT) {
        ...
        case SettingConstants.ROW_SETTING_SLOW_MOTION:
            ...
            if(ProjectConfig.FEATURE_CAMERA_HIDE_SLOWMOTION_ICON){
                removePreference = true;
            }
            break;
        ...
        case SettingConstants.ROW_SETTING_DNG:
            ...
            break;
        //add by zhangxiulu 20150119 begin
        case SettingConstants.ROW_SETTING_CAMERA_SHUTTER_SOUND:
            break;
        //add by zhangxiulu 20150119 end

        //Add by zhangtao for timestamp (start)
        case SettingConstants.ROW_SETTING_CAMERA_TIME_STAMP:
            if (!com.xchengtech.ProjectConfig.XC_CAMERA_TIME_STAMP_SUPPORT) {
                removePreference = true;
            }
            break;
        //Add by zhangtao for timestamp (end)
