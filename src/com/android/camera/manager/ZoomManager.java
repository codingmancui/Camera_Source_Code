...
public class ZoomManager extends ViewManager implements
    ...
    private int getMaxZoomRatio() {
        int ratio = UNKNOWN;
        if (mZoomRatios != null) {
            if(ProjectConfig.FEATURE_CAMERA_ZOOM_DOUBLE_SUPPORT){
            //XCSW Modify by wucaihong SWELE-254 start
                ratio = mZoomRatios.get(mZoomRatios.size() - 6);
            }else{
                ratio = mZoomRatios.get(mZoomRatios.size() - 1);
            }
            //XCSW Modify by wucaihong SWELE-254 end
        }
        return ratio;
    }
