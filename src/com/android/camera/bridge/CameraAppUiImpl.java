...
//chendonglei add for fp capture 20160809 start
...
//chendonglei add for fp capture 20160809 end
...
public class CameraAppUiImpl implements ICameraAppUi {
	...
	//chendonglei add for fp capture 20160809 start
	private static final String TAGFINGER = "CameraFingerprint";
    ...
    public void createCommonView() {
		...
		//chendonglei add for fp capture 20160809 start
		if(com.xchengtech.ProjectConfig.XC_FP_CAPTURE_SPT){
		int result = 0;
		Log.i("abcd", "camera is start work");
		try {
			result = Settings.System.getInt(
					mCameraActivity.getContentResolver(), "camera");
		} catch (SettingNotFoundException e) {

			e.printStackTrace();
		}
		if (result == 1) {
			mFingerprintEnable = true;
			Log.i("abcd", "camera is start work");
	...
	//chendonglei add for fp capture 20160809 start
		if ((mFingerprintEnable)&&com.xchengtech.ProjectConfig.XC_FP_CAPTURE_SPT) {
			KeyguardManager mKeyguardManager = (KeyguardManager) mCameraActivity
					.getSystemService(Context.KEYGUARD_SERVICE);
			
				if ((state == ViewState.VIEW_STATE_CAMERA_OPENED)) {
					
					startListener();
				} else if (state == ViewState.VIEW_STATE_CAMERA_CLOSED&&!mKeyguardManager.inKeyguardRestrictedInputMode()) {
					 stopListener();
				}
	...
	public void startListener() {
		...
		mFpEtsManager.enableTouchListen(5000);
		mFpEtsManager.startListening(new FingerprintReceiver() {

			@Override
			public void onFingerprintEvent(int arg0, int arg1, int arg2,
					Object arg3) {
				Log.i("abcd", "camera is start work");
				mFpEtsManager.disableTouchListen();
				takePhoto();
