...
public class CameraDeviceCtrl implements SurfaceHolder.Callback {
    ...
    public void onPause() {
        ...
        if(ProjectConfig.FEATURE_CAMERA_DEL_BLACKSCR_GOGALLERY){
            //
        }else{
            showRootCover();
    ...
    public void switchCamera(int cameraId) {
        ...
        //Add by zhangtao for zoom to platform (start)
        mCameraAppUi.resetZoom();
        //Add by zhangtao for zoom to platform (end)
    ...
    public void applyParameters(boolean isNeedRestart) {
        ...
        if (needRestart) {
            ...
            if(ProjectConfig.FEATURE_CAMERA_ZSD_DEFAULT_ON){
                int curr_mode = mCameraActivity.getCurrentMode();
                String default_value = SettingUtils.getPreferenceValue(mCameraActivity,
                    mPreferences,SettingConstants.ROW_SETTING_ZSD, "on");
                if(curr_mode == ModePicker.MODE_PANORAMA || curr_mode == ModePicker.MODE_PHOTO_PIP){
                    default_value = "off";
                }
                mCurCameraDevice.getParametersExt().setZSDMode(default_value);
                //Log.e(TAG, "HelloWorld defaultValue = " + mCurCameraDevice.getZsdMode());
            }
        }
        if (getCameraState() == CameraState.STATE_OPENING_CAMERA) {
    ...
    private void applyFirstParameters () {
        ...
        if(ProjectConfig.FEATURE_CAMERA_ZSD_DEFAULT_ON){
            mCurCameraDevice.getParametersExt().setZSDMode(SettingUtils.getPreferenceValue(mCameraActivity,
                    mPreferences,SettingConstants.ROW_SETTING_ZSD, "on"));
            //Log.e(TAG, "HelloWorld currentValue = " + mCurCameraDevice.getZsdMode());
        }else if (!mCameraActivity.isImageCaptureIntent() && !mCameraActivity.isVideoCaptureIntent()) {
