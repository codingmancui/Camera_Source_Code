...
public class FileSaver {
    ...
    private class PhotoOperator extends RequestOperator {
        ...
        public synchronized void saveRequest() {
            ...
            //Add by zhangtao for timestamp (start)
            if (ProjectConfig.XC_CAMERA_TIME_STAMP_SUPPORT && isTimeStampOn()) {
        ...
        private boolean isTimeStampOn() {
            return "on".equals(mContext.getISettingCtrl().getSettingValue(SettingConstants.KEY_CAMERA_TIME_STAMP));
        }

        public Bitmap createBitmap(Bitmap src, long dateTaken) {
            int paddingBottom = 10;
            String datetime = DateFormat.format("kk:mm dd-MM-yyyy", dateTaken).toString();
            int w = src.getWidth();
            int h = src.getHeight();
            ...
            int textColor = 0xffffff93;
            int shadowColor = 0xffff8a15;
            Typeface font = Typeface.create(fontName, Typeface.NORMAL);
            final float density = mContext.getResources().getDisplayMetrics().density;
            int textSize = (int)(60 * density);
            p.setTextSize(textSize);
            p.setColor(0xffffff93);
            p.setTypeface(font);
            //p.setShadowLayer(1, 0, 0, shadowColor);
            canvas.drawBitmap(src, 0, 0, p);
            canvas.drawText(datetime, w / 2, h - paddingBottom, p);
