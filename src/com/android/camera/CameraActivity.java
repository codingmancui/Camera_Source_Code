...
public class CameraActivity extends ActivityBase implements
    ...
    public void onCreate(Bundle icicle) {
        ...
        //add by liyafeng for slideKey 20160818 (start)
        if (ProjectConfig.FEATURE_SLIDEKEY_OPEN_FRONT_CAMERA) {
            Intent intent = getIntent();
            boolean fromSlideKey = intent.getBooleanExtra("start_from_slidekey", false);
            if (fromSlideKey) {
                SettingUtils.writePreferredCameraId(mPreferences.getGlobal(), 1);
            }
        }
        //add by liyafeng for slideKey 20160818 (end)
    ...
    public boolean onKeyDown(int keyCode, KeyEvent event) {
        ...
        //wangleiquan for menu key SWYYW-716(start)
        if (KeyEvent.KEYCODE_MENU == keyCode) {
            if (ProjectConfig.FEATURE_MENUKEY_CLICK_TO_RECENT) {
                return true;
            } else if (isFullScreen() && event.getRepeatCount() == 0
                    && mCameraAppUi.performSettingClick()) {
                return true;
            }
        }
        //wangleiquan for menu key SWYYW-716(end)
    ...
    public boolean onKeyUp(int keyCode, KeyEvent event) {
        //caowen add for fix BUG-ID:SWYYWAZUMI-74 press KEYCODE_BACK save video 20160825 start
        if (keyCode == KeyEvent.KEYCODE_BACK) {
            return super.onKeyUp(keyCode, event);
        }
        //caowen add for fix BUG-ID:SWYYWAZUMI-74 press KEYCODE_BACK save video 20160825 end
        if (FeatureSwitcher.isApi2Enable(this)) {
        ...
        //wangleiquan for menu key SWYYW-716(start)
        if (KeyEvent.KEYCODE_MENU == keyCode) {
            if (ProjectConfig.FEATURE_MENUKEY_CLICK_TO_RECENT && isFullScreen()
                    && mCameraAppUi.performSettingClick()) {
                return true;
            }
        }
    ...
    private static final String CUSTOM_GALLERY_PACKAGE = "com.android.gallery3d";
    private static final String CUSTOM_GALLERY_ACTIVITY = "com.android.gallery3d.app.GalleryActivity";
    ...
    public void gotoGallery() {
        ...
        if(ProjectConfig.FEATURE_CAMERA_CUSTOM_GALLERY_SUPPORT){
            String customerPackage = SystemProperties.get("ro.gallery.packagename", CUSTOM_GALLERY_PACKAGE);
            String customerActivity = SystemProperties.get("ro.gallery.activityname", CUSTOM_GALLERY_ACTIVITY);
        ...
        public void onRestorePreferencesClicked() {
            Runnable runnable = new Runnable() {
                ...
                public void run() {
                    ...
                    //XCSW add by zhangtao for default preview ratio 20160817 (start)
                    ...
                    SettingUtils.restorePreviewRatioToDefault(
                            ...
                            getSharePreferences(frontCameraId),
                            frontCameraId);
                    //XCSW add by zhangtao for default preview ratio 20160817 (end)
    ...
    // add by guozhengwei for sideslipkey start
    private BroadcastReceiver mDemoReceiver = new BroadcastReceiver() {
        @Override
        public void onReceive(Context context, Intent intent) {
            String action = intent.getAction();
            if ("android.intent.action.switchcamera".equals(action)) {
                int mCameraId;
                int cameraId = mCameraDeviceCtrl.getCameraId();
                Log.i(TAG, "mDemoReceiver ... cameraId" + cameraId);
                if (cameraId == 0) {
                    cameraId = 1;
                } else {
                    cameraId = 0;
    ...
    // add by guozhengwei for sideslipkey end
    ...
    private void installIntentFilter() {
        ...
        demoFilter.addAction("android.intent.action.switchcamera");
        registerReceiver(mDemoReceiver, demoFilter);
        // add by guozhengwei for sideslipkey end
    }
    ...
    private void uninstallIntentFilter() {
        ...
        // add by guozhengwei for sideslipkey start
        unregisterReceiver(mDemoReceiver);
        // add by guozhengwei for sideslipkey end
    }
